...
{
  return PVector.mult(PVector.sub(xa,xb), -k);
}
...
void draw()
{
  ...
  for (int i = 0; i < nNodes; i++)
  {
    ...
    {
      //Collision detection: compute penetration depth
      float delta = PVector.dot(wallNormal[j], PVector.sub(wallPoint[j], nodePos[i]));
      //Collision response; apply penalty force
      if(delta>0)
      {
    ...
    if (mouseId != -1)
    {
      nodeForce[mouseId].add(SpringElasticForceZeroLength(nodePos[mouseId], mousePos, mouseStiffness));
    }
